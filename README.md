...
On power-up, the firmware performs the following initialization:
    ...
    Hardware Initialization: Configure all peripherals (timers, USART, ADC, SPI, interrupts)
    LCD Initialization: Initialize 4-bit LCD interface, backlight on
    Serial Initialization: Configure USART and encryption state
    Load Configuration: Read saved settings from EEPROM
    DAC Test (dacTest()): Verify SPI communication and LTC1661 DAC output
        If DAC test fails: displays "DAC FAIL - HALT / Check hardware" and halts
    FET Calibration (fetCalibrate()): Verify H-bridge FET pairs are functional
    ...
    Initialization Progress Bar: Displays "Initializine..." with a 16-step progress bar
    ...
    Menu Startup: Shows main menu (mode name + navigation hint)
    Select Mode: Loads last saved mode from EEPROM
    Enable Interrupts: sei() starts ISR-driven pulse generation
    Enable Watchdog: Activates 2-second watchdog timer
    First Update: Calls mode_dispatcher_update() to initialize output state
...
Compilation & Upload
    ...
    Open MK312BT.ino in Arduino IDE
    Select Tools → Board → MightyCore → ATmega16
    Select Tools → Clock → 8 MHz external
    Select Tools → Programmer (e.g., USBasp, Arduino as ISP)
...
Output Generation Flow
    ...
    Mode/parameter engine updates gate, frequency, and width values
    Main loop converts frequency index to period (us) and width index to pulse width (us)
    Main loop calls pulse_set_width_a/b(), pulse_set_frequency_a/b(), pulse_set_gate_a/b()
    Timer ISRs autonomously generate biphasic pulse trains at the configured rate
    Main loop reads level pots via ADC and writes intensity to DAC via SPI
    DAC output voltage controls the transformer drive level (overall power)
...
Interrupt Priorities (AVR hardware order)
    ...
    INT0/INT1 - External audio inputs
    TIMER2_COMP - Channel B H-bridge pulse generation
    TIMER1_COMPA - Channel A H-bridge pulse generation
    USART_RXC - Serial receive buffer drain
    SPI_STC - SPI buffer drain
...
Access system settings:
    ...
    Toggle Channel: Switch between Channel A and B for parameter editing
    Power Level: Set Low/Normal/High power output
    Set Favorite: Save current mode as favorite
...
The firmware supports the following operational modes:
    ...
    Waves - Smooth sine wave modulation
    Stroke - Rhythmic pulsing pattern
    Climb - Gradually increasing intensity
    Combo - Combined pattern mode
    Intense - High-frequency stimulation
    Rhythm - Musical beat patterns
    Audio1/2/3 - Audio-responsive modes
    Random1/2 - Randomized pattern selection
    Toggle - On/off switching
    Orgasm - Progressive intensity build
    Torment - Intense variable pattern
...
To add or modify modes:
    ...
    Create new program block in program_blocks.c
    Add entry to program_block_table[]
    Map mode number in mode_dispatcher.c
    Define mode constant in mode_dispatcher.h
